...
public:
    ...
    const double                                                 tol = 1e-6)
  {
    ...
    // TODO: loop could be eliminated if we would use a single RPE
    for(const auto & face_pair : face_pairs)
    {
    ...
    const dealii::VectorTools::EvaluationFlags::EvaluationFlags                   vec_flags) const
  {
    ...
    // TODO: loop could be eliminitated if we would use a single RPE
...
private:
  ...
  template<typename T1, std::size_t n_lanes, int dim_>
  void
  copy_data(dealii::Tensor<1, dim_, dealii::VectorizedArray<T1, n_lanes>> & dst,
            const unsigned int                                              v,
            const dealii::Tensor<1, dim_, T1> &                             src) const
  {
    AssertIndexRange(v, n_lanes);
  ...
  template<typename T1, std::size_t n_lanes, int n_components_, int dim_>
  void
  copy_data(dealii::Tensor<1,
                           n_components_,
                           dealii::Tensor<1, dim_, dealii::VectorizedArray<T1, n_lanes>>> & dst,
            const unsigned int                                                              v,
            const dealii::Tensor<1, n_components_, dealii::Tensor<1, dim_, T1>> & src) const
  {
