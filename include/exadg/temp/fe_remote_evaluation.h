...
struct PrecomputedEvaluationData
{
  ...
  using gradient_type = typename internal::FEPointEvaluation::
    EvaluatorTypeTraits<dim, dim, n_components, value_type_>::real_gradient_type;
...
class PrecomputedEvaluationDataAccessor
{
  ...
  using gradient_type =
    typename PrecomputedEvaluationData<dim, n_components, value_type_>::gradient_type;
...
public:
   ...
   * object if remote points are related to matrix-free cell batches.
   */
  void
  reinit_cells(const FERemoteCommunicationObjectEntityBatches<dim> & comm_object,
   ...
   */
  void
  reinit_faces(const std::vector<FERemoteCommunicationObjectEntityBatches<dim>> & comm_objects,
   ...
   */
  void
  reinit_faces(const std::vector<FERemoteCommunicationObject<dim>> & comm_objects,
  ...
  void
  reinit_faces(const std::vector<FERemoteCommunicationObjectTwoLevel<dim>> & comm_objects,
  ...
  template<int n_components,
           ...
           typename VectorType>
  void
  update_ghost_values(PrecomputedEvaluationDataType &                     dst,
                      const MeshType &                                    mesh,
...
private:
  ...
  public:
    ...
    copy_data(
      ...
      AlignedVector<T1> &                             dst,
      const std::vector<T2> &                         src,
      const std::vector<std::pair<typename Triangulation<dim>::cell_iterator, unsigned int>> &
        cell_face_nos);
  ...
  private:
    ...
    static void
    copy_data_entries(VectorizedArray<T1, n_lanes> & dst, const unsigned int v, const T1 & src);
    ...
    static void
    copy_data_entries(
      Tensor<rank_, n_components_, Tensor<rank_, dim_, VectorizedArray<T1, n_lanes>>> & dst,
      const unsigned int                                                                v,
...
compute_remote_communicator_faces_point_to_point_interpolation(
  ...
  const std::vector<std::pair<types::boundary_id, std::function<std::vector<bool>()>>> &
...
public:
  ...
  void
  gather_evaluate(const VectorType & src, const EvaluationFlags::EvaluationFlags flags);
...
PrecomputedEvaluationDataView::get_shift(const unsigned int index) const
{
  ...
  Assert(index != numbers::invalid_unsigned_int, ExcMessage("Index has to be valid!"));
                                         ...
                                         const unsigned int face_number) const
{
  ...
  Assert(cell_index != numbers::invalid_unsigned_int, ExcMessage("Cell index has to be valid!"));
  Assert(face_number != numbers::invalid_unsigned_int, ExcMessage("Face number has to be valid!"));
...
PrecomputedEvaluationDataAccessor<dim, n_components, value_type_>::
  PrecomputedEvaluationDataAccessor(
    const PrecomputedEvaluationData<dim, n_components, value_type_> & data,
    const PrecomputedEvaluationDataView &                             view)
  : view(view), data(data), data_offset(numbers::invalid_unsigned_int)
{
}

template<int dim, int n_components, typename value_type_>
const typename PrecomputedEvaluationData<dim, n_components, value_type_>::value_type
PrecomputedEvaluationDataAccessor<dim, n_components, value_type_>::get_value(
  const unsigned int q) const
{
  Assert(data_offset != numbers::invalid_unsigned_int, ExcMessage("reinit() not called."));
  AssertIndexRange(data_offset + q, data.values.size());
  return data.values[data_offset + q];
}

template<int dim, int n_components, typename value_type_>
const typename PrecomputedEvaluationData<dim, n_components, value_type_>::gradient_type
PrecomputedEvaluationDataAccessor<dim, n_components, value_type_>::get_gradient(
  const unsigned int q) const
{
  Assert(data_offset != numbers::invalid_unsigned_int, ExcMessage("reinit() not called."));
...
template<int dim, int n_components, typename value_type_>
void
PrecomputedEvaluationDataAccessor<dim, n_components, value_type_>::reinit(const unsigned int index)
{
...
template<int dim>
std::vector<std::pair<unsigned int, unsigned int>>
FERemoteCommunicationObjectEntityBatches<dim>::get_communication_object_pntrs() const
{
...
template<int dim>
void
FERemoteEvaluationCommunicator<dim>::reinit_cells(
  const FERemoteCommunicationObjectEntityBatches<dim> & comm_object,
  const std::pair<unsigned int, unsigned int> &         cell_batch_range,
  const std::vector<unsigned int> &                     quadrature_sizes)
{
  communication_objects.clear();
  communication_objects.push_back(comm_object);

  // fetch points and update communication patterns
  const unsigned int n_cells = quadrature_sizes.size();
  AssertDimension(n_cells, cell_batch_range.second - cell_batch_range.first);

  // construct view:
  view.start = cell_batch_range.first;
  ...
  {
    view.ptrs[cell + 1] = view.ptrs[cell] + quadrature_sizes[cell];
  }
}
...
FERemoteEvaluationCommunicator<dim>::reinit_faces(
  const std::vector<FERemoteCommunicationObjectEntityBatches<dim>> & comm_objects,
  const std::pair<unsigned int, unsigned int> &                      face_batch_range,
  const std::vector<unsigned int> &                                  quadrature_sizes)
{
  ...
  const std::vector<std::vector<unsigned int>> &                quadrature_sizes)
{
  ...
  for(const auto & cell : cell_iterator_range)
  {
    ...
    {
      const unsigned int face_index = cell_ptrs[cell->active_cell_index()] + f;

      face_ptrs[face_index + 1] =
        face_ptrs[face_index] + quadrature_sizes[cell->active_cell_index()][f];
    }
  }
}
  ...
  for(const auto & communication_object : communication_objects)
  {
    if(eval_flags & EvaluationFlags::values)
    {
      std::visit(
        [&](const auto & obj)
        {
          CopyInstructions::copy_data(view,
                                      dst.values,
  ...
  const std::vector<unsigned int> &               indices)
{
  ...
  {
    for(unsigned int j = view.get_shift(idx); j < view.get_shift(idx + 1); ++j, ++c)
    {
  ...
  const std::vector<std::pair<unsigned int, unsigned int>> & batch_id_n_entities)
{
  ...
  for(const auto & [batch_id, n_entries] : batch_id_n_entities)
  {
    for(unsigned int v = 0; v < n_entries; ++v)
      for(unsigned int j = view.get_shift(batch_id); j < view.get_shift(batch_id + 1); ++j, ++c)
      {
        AssertIndexRange(j, dst.size());
        AssertIndexRange(c, src.size());
...
template<typename T1, typename T2>
void
FERemoteEvaluationCommunicator<dim>::CopyInstructions::copy_data_entries(T1 &,
                                                                         const unsigned int,
                                                                         const T2 &)
{
  ...
  const double       tolerance)
{
  ...
  std::vector<unsigned int> global_quadrature_sizes(matrix_free.n_boundary_face_batches(),
  ...
  for(const auto & [nm_face, marked_vertices] : non_matching_faces_marked_vertices)
  {
    // Construct the communication object for every face ID:
    // 1) RemotePointEvaluation with user specified function for marked
    // vertices.
    auto rpe = std::make_shared<Utilities::MPI::RemotePointEvaluation<dim>>(tolerance,
                                                                            false,
                                                                            0,
                                                                            marked_vertices);

    // 2) Face batch IDs and number of faces in batch.
    std::vector<std::pair<unsigned int, unsigned int>> face_batch_id_n_faces;
    ...
    FEFaceEvaluation<dim, -1, 0, 1, Number> phi(matrix_free, true, dof_no, quad_no);

    // Iterate over the boundary faces.
    for(unsigned int bface = 0; bface < face_batch_range.second - face_batch_range.first; ++bface)
    {
      ...
      if(matrix_free.get_boundary_id(face) == nm_face)
      {
        ...
        const unsigned int n_faces = matrix_free.n_active_entries_per_face_batch(face);
        face_batch_id_n_faces.emplace_back(std::make_pair(face, n_faces));
        ...
        Assert(global_quadrature_sizes[bface] == numbers::invalid_unsigned_int,
  ...
  remote_communicator.reinit_faces(comm_objects, face_batch_range, global_quadrature_sizes);
  ...
  const double                                 tolerance)
{
  ...
  std::vector<Quadrature<dim - 1>> global_quadrature_vector(
    (matrix_free.n_inner_face_batches() + matrix_free.n_boundary_face_batches()) * n_lanes);
  ...
  for(const auto & [nm_face, marked_vertices] : non_matching_faces_marked_vertices)
  {
    ...
    auto intersection_data =
      GridTools::internal::distributed_compute_intersection_locations<dim - 1>(
        cache, intersection_requests, global_bboxes, marked_vertices(), tolerance);
    ...
    rpe->reinit(
      intersection_data.template convert_to_distributed_compute_point_locations_internal<dim>(
        n_q_pnts_1D, tria, mapping, &mapped_quadratures_recv_comp),
      tria,
      mapping);
    ...
    for(unsigned int i = 0; i < intersection_requests.size(); ++i)
    {
          ...
          ++ptr)
      {
        ...
        std::transform(ps.begin(),
                       ...
                       [&](const Point<dim> & p)
                       { return mapping.project_real_point_to_unit_point_on_face(cell, f, p); });
  ...
  std::vector<unsigned int> global_quadrature_sizes(global_quadrature_vector.size());
  ...
  if(nm_mapping_info != nullptr)
  {
    ...
    for(; face_batch < matrix_free.n_inner_face_batches(); ++face_batch)
    {
      for(unsigned int v = 0; v < n_lanes; ++v)
      {
        if(v < matrix_free.n_active_entries_per_face_batch(face_batch))
          vector_face_accessors.push_back(matrix_free.get_face_iterator(face_batch, v));
        else
          vector_face_accessors.push_back(matrix_free.get_face_iterator(face_batch, 0));
      }
    }
    // and boundary face batches
    for(; face_batch < (matrix_free.n_inner_face_batches() + matrix_free.n_boundary_face_batches());
        ++face_batch)
    {
      for(unsigned int v = 0; v < n_lanes; ++v)
      {
    ...
    nm_mapping_info->reinit_faces(vector_face_accessors, global_quadrature_vector);
  }
  ...
  const EvaluationFlags::EvaluationFlags flags)
{
  if(tria)
  {
    ...
    comm->template update_ghost_values<n_components>(
      this->data, *tria, src, flags, first_selected_component, evaluation_flags);
  }
  else if(dof_handler)
  {
    comm->template update_ghost_values<n_components>(
      this->data, *dof_handler, src, flags, first_selected_component, evaluation_flags);
  }
  else
...
{
  internal::PrecomputedEvaluationDataAccessor data_accessor(data, comm->get_view());
  return data_accessor;
}

template<int dim, int n_components, typename value_type>
void
FERemoteEvaluation<dim, n_components, value_type>::set_mesh(const Triangulation<dim> & tria)
{
  this->tria = &tria;
}
...
template<int dim, int n_components, typename value_type>
void
FERemoteEvaluation<dim, n_components, value_type>::set_mesh(const DoFHandler<dim> & dof_handler)
{
