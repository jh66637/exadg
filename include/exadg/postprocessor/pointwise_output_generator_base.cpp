  ...
  PointwiseOutputDataBase<dim> const & pointwise_output_data_in)
{
  ...
  if(pointwise_output_data.write_output && pointwise_output_data.evaluation_points.size() > 0)
  {
...

    {
      ...
      fill_evaluation_points("GeneralInformation/EvaluationPoints");
    }
...
template<int dim, typename Number>
void
PointwiseOutputGeneratorBase<dim, Number>::add_scalar_quantity_dataset(std::string const & name)
{
#ifdef DEAL_II_WITH_HDF5
  auto group = hdf5_file->open_group("PhysicalInformation");
  group.template create_dataset<Number>(
    name, std::vector<hsize_t>{pointwise_output_data.evaluation_points.size(), 1 * n_out_samples});
#else
  (void)name;
  AssertThrow(false, dealii::ExcMessage("deal.II is not compiled with HDF5!"));
#endif
...
template<int dim, typename Number>
void
PointwiseOutputGeneratorBase<dim, Number>::add_vector_quantity_datasets(std::string const & name)
{
  for(unsigned int d = 0; d < dim; ++d)
    add_scalar_quantity_dataset(name + std::to_string(d));
}
...
template<int dim, typename Number>
void
PointwiseOutputGeneratorBase<dim, Number>::fill_evaluation_points(std::string const & name)
{
#ifdef DEAL_II_WITH_HDF5
