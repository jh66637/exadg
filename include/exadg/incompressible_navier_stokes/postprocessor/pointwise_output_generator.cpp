...
PointwiseOutputGenerator<dim, Number>::PointwiseOutputGenerator(MPI_Comm const & comm)
  : PointwiseOutputGeneratorBase<dim, SolutionContainer<Number>>(comm)
{
}
...
template<int dim, typename Number>
void
PointwiseOutputGenerator<dim, Number>::do_evaluate(SolutionContainer<Number> const & solution)
{
  if(pointwise_output_data.write_pressure)
  {
    auto const values =
      this->template compute_point_values<1>(solution.pressure, *dof_handler_pressure);
    this->write_quantity("Pressure", values);
  }
  if(pointwise_output_data.write_velocity)
  {
    auto const values =
      this->template compute_point_values<dim>(solution.velocity, *dof_handler_velocity);
