                                    ...
                                    Number const                             time)
{
  ...
  else if(boundary_type == BoundaryType::Admittance)
  {
    ...
    auto const um = velocity_m.get_value(q);
    auto const pm = pressure_m.get_value(q);

    auto const um_normal     = (um * n) * n;
    auto const um_tangential = um - um_normal;

    // normal share of velocty
    auto const u_n = (Number{2.0} * Y * pm / c) * n - um_normal;

    // tangential share of velocity stays the same
    return u_n + um_tangential;
  }
  else
  {
    AssertThrow(false, dealii::ExcMessage("Boundary type of face is invalid or not implemented."));
